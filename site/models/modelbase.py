...
from ..utils import place_nulls, subdict, deep_group
from datetime import datetime
from sqlalchemy.ext.associationproxy import (
    _AssociationDict, _AssociationList)
from sqlalchemy.orm.collections import (
    InstrumentedList, MappedCollection)
from ..json_plus import json_encoder


def parent_classes(obj):
    return (cls for cls in obj.__class__.mro()
            if cls not in [ModelBase, Model, object])
...
class ModelBase(Model):
    ...
    def _isinstance(self, model, raise_error=True):
        """Taken from Overholt Service class
        """
    ...
    @classmethod
    def parents(cls):
        return (c for c in cls.mro()
                if c not in [ModelBase, object])

    def column_keys(self):
        keys = self.__table__.columns.keys()
        for cls in parent_classes(self):
            if hasattr(cls, '__table__'):
                keys = chain(keys, cls.__table__.columns.keys())
